...
class CharacterChatBot:
    ...
    def chat(self, message, history):
        # eg history = [["hi, how are you?", "i'm fine i am system"],[]]

        messages = []
        # Add the system prompt
        messages.append(
            {"role": "system", "content": """"Your are Naruto from the anime "Naruto". Your responses should reflect his personality and speech patterns \n"""})

        for message_and_respnse in history:
            messages.append(
                {"role": "user", "content": message_and_respnse[0]})
            messages.append(
                {"role": "assistant", "content": message_and_respnse[1]})

        messages.append({"role": "user", "content": message})

        # to terminate the generation if the model output these
        terminator = [
            self.model.tokenizer.eos_token_id,
            self.model.tokenizer.convert_tokens_to_ids("<|eot_id|>")
        ]

        # do sample gives slightly different response every time we run it
        output = self.model(
            messages,
            max_length=256,
        ...
        # get the output at the end which is the latest response
    ...
    def train(self,
        ...
        # making the model 4 bits
        # to make it easier to train and need less gpu
        bnb_config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_quant_type="nf4",
            bnb_4bit_compute_dtype=torch.float16,
