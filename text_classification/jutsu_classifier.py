...
class JutsuClassifier():
    def __init__(self,
        ...
        if not huggingface_hub.repo_exists(self.model_path):
            ...
            # fixing the skewed data set for the jutsu classification
            train_data_df = train_data.to_pandas()
            test_data_df = test_data.to_pandas()

            all_data = pd.concat(
                [train_data_df, test_data_df]).reset_index(drop=True)
            # it is just prefered to have the whole data for getting the class weights
    ...
    def load_data(self, data_path):
        ...
        tokenized_train = train_dataset.map(lambda examples: self.preprocess_function(
            self.tokenizer, examples), batched=True)  # batched to prevent it from
    ...
    def postprocess(self, model_output):
        ...
        for pred in model_output:
            # jutsu with the maximum score will be our prediction eg either genjutsu, ninjutsu or taijutsu
